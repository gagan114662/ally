#!/usr/bin/env python3
"""
Finnhub offline tests - deterministic mock data and offline behavior
"""
...
class TestFinnhubOffline:
    ...
    def test_different_symbols_different_data(self):
        ...
        with patch.dict(os.environ, {"ALLY_LIVE": "0"}):
            ...
            # Base prices should be different (140 + i * 20)
            assert abs(aapl_close - 140) < 50  # Around 140
            assert abs(msft_close - 160) < 50  # Around 160
            assert abs(googl_close - 180) < 50  # Around 180
    ...
    def test_metadata_attributes(self):
        ...
        with patch.dict(os.environ, {"ALLY_LIVE": "0"}):
            ...
            assert df.attrs['provider'] == 'finnhub_mock'
    ...
    def test_tool_level_offline_integration(self):
        ...
        with patch.dict(os.environ, {"ALLY_LIVE": "0"}):
            ...
            assert panel_meta['source'] == 'finnhub'
            ...
            assert provider_info['AAPL']['provider'] == 'finnhub_mock'
            assert provider_info['MSFT']['provider'] == 'finnhub_mock'
    ...
    def test_finnhub_volume_distinct_from_other_providers(self):
        """Test that Finnhub mock data has distinct volume characteristics (1.8M base vs 2M Polygon/1M Alpha Vantage)"""
        with patch.dict(os.environ, {"ALLY_LIVE": "0"}):
            adapter = FinnhubAdapter()
            ...
            # Finnhub should have medium base volume (1.8M between Alpha Vantage's 1M and Polygon's 2M)
            avg_volume = df['volume'].mean()
            assert 1200000 < avg_volume < 2500000, f"Expected Finnhub volume range, got {avg_volume}"
    
    def test_finnhub_unique_price_characteristics(self):
        """Test that Finnhub mock data has unique base price and volatility"""
        with patch.dict(os.environ, {"ALLY_LIVE": "0"}):
            adapter = FinnhubAdapter()
            ...
            # Base price should be around 140 (different from Alpha Vantage's 120 and Polygon's 150)
            avg_close = df['close'].mean()
            assert 120 < avg_close < 180, f"Expected Finnhub price range around 140, got {avg_close}"
            
            # Check that seed generates consistent but different data
            assert df.attrs['provider'] == 'finnhub_mock'
